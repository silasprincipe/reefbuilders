...
#Read species data
sp <- read.csv("data/moca/moca_final.csv")
...
r[cellFromXY(r, sp[sp$moca==0,1:2])] <- 0
r[cellFromXY(r, sp[sp$moca==1,1:2])] <- 1
...
#Change column names
colnames(data) <- c("decimalLongitude", "decimalLatitude", "moca")

#Remove NAs
sp <- filter(data, moca == 1 | moca == 0)

#Plot just to verify
plot(wrld_simpl, xlim = c(-99,22), ylim=c(-42.5,42.5))
points(sp[sp$moca == 1,1:2], col = "blue", pch = 20, cex = 0.75)
points(sp[sp$moca == 0,1:2], col = "red", pch = 20, cex = 0.75)

#Create a 'temp' object with the final results
# sp.total <- sp
# colnames(sp.total) <- c("decimalLongitude", "decimalLatitude", "Siderastrea")

write.csv(sp, "data/moca/moca_cell.csv", row.names = F)
