...
#Read species data
sp <- read.csv("data/muhi/muhi_final.csv")
...
r <- raster(nrow=1020, ncol=840, xmn = -99, xmx= -29, ymn= -42.5, ymx = 42.5)
values(r) <- NA
...
r[cellFromXY(r, sp[sp$Mussismilia_hispida==0,1:2])] <- 0
r[cellFromXY(r, sp[sp$Mussismilia_hispida==1,1:2])] <- 1
...
#Change column names
colnames(data) <- c("decimalLongitude", "decimalLatitude", "muhi")

#Remove NAs
sp <- filter(data, muhi == 1 | muhi == 0)

#Plot just to verify
plot(wrld_simpl, xlim = c(-99,-29), ylim=c(-42.5,42.5))
points(sp[sp$muhi == 1,1:2], col = "blue", pch = 20, cex = 0.75)
points(sp[sp$muhi == 0,1:2], col = "red", pch = 20, cex = 0.75)

#Create a 'temp' object with the final results
# sp.total <- sp
# colnames(sp.total) <- c("decimalLongitude", "decimalLatitude", "Siderastrea")

write.csv(sp, "data/muhi/muhi_cell.csv", row.names = F)
