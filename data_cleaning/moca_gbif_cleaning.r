...
#Individual count
table(sp.cl$individualCount, useNA= 'ifany')
sp.cl <- sp.cl %>%
  filter(individualCount > 0 | is.na(individualCount)) %>%
  filter(individualCount <= 100 |
...
inst.code
sp.cl <- sp.cl %>%
  filter(institutionCode != "iNaturalist") %>% #iNaturalist
  filter(institutionCode != "BioObs") %>% #BioObs
  filter(institutionCode != "Diveboard") %>% #Diveboard
  filter(institutionCode != "NO DISPONIBLE") %>% #No disponible
  filter(institutionCode != "CardObs - UMS PatriNat") %>% #CardObs
  filter(institutionCode != "SEAK") %>% #SEAK
  filter(institutionCode != "MW") %>%
  filter(institutionCode != "Moscow State University") #Moscow State University: although
                                           #the source inst. is reliable, 
                                           #the aim of this dataset
                                           #was a review of copepods on Scleractineans.
...
#Full data
write.csv(sp.cl, file = "data_cleaning/moca_gbif_complete.csv",
          row.names = FALSE)


#Generates the final list of occurence points ready for BIOMOD2
sp.cl$pa <- rep(1, length.out = length(sp.cl$species))
sp.points <- sp.cl %>% dplyr::select(decimalLongitude,
                              decimalLatitude,
                              pa)
colnames(sp.points)[3] <- "Montastraea_cavernosa"
write.csv(sp.points, file = "data_cleaning/moca_gbif.csv",
          row.names = FALSE)
